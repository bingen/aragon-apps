...
contract Payroll is App, Initializable, ERC677Receiver {
     ...
     */
    function initialize(Finance _finance, EtherToken _etherToken, address _usdToken, address _oracleAddress) external onlyInit {
        initialized();
    ...
    {
        _addEmployeeWithName(accountAddress, allowedTokens, initialYearlyUSDSalary, "");
    }
    ...
    function addEmployeeWithName(
        ...
        auth(OWNER_WRITE_ROLE)
    {
        _addEmployeeWithName(accountAddress, allowedTokens, initialYearlyUSDSalary, name);
    }
    ...
    function addFunds() public payable {
        ...
        LogFund(msg.sender, address(etherToken), msg.value, etherToken.balanceOf(this), "");
    }
    ...
    function escapeHatch() public auth(OWNER_WRITE_ROLE) {
        for (uint i = 0; i < globalTokensArray.length; i++) {
            ...
            uint256 tokenBalance = tokenContract.balanceOf(finance);
            // Avoid zero balances
            if (tokenBalance == 0)
                continue;
            // send balance to owner
            //require(tokenContract.transfer(employer, tokenBalance));
            finance.newPayment(tokenContract, employer, tokenBalance, uint64(getTimestamp()-1), 0, 1, "");
            LogFund(employer, token, tokenBalance, tokenContract.balanceOf(this), "");
        }
    ...
    function payday() public auth(EMPLOYEE_ROLE) {
        ...
        for (uint i = 0; i < employee.allowedTokensArray.length; i++) {
            ...
            ERC20 tokenContract = ERC20(token);
            finance.newPayment(tokenContract, msg.sender, tokenAmount, uint64(getTimestamp()-1), 0, 1, "");
