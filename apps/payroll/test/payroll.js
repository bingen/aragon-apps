...
contract('Payroll', function(accounts) {
  ...
  const deployErc20Token = async (name="ERC20Token") => {
    ...
    await token.generateTokens(payroll.address, new web3.BigNumber(10**9).times(new web3.BigNumber(10**18)));
    return token;
  };
  ...
  it("Deploying and initializing contract", async () => {
    ...
    for (let i = 1; i < 9; i++)
      await payroll.addFunds.sendTransaction({ from: accounts[i], to: payroll.address, value: web3.toWei(90, 'ether') });
  });
  ...
  it("Deploying tokens", async () => {
    ...
    const setAndCheckRate = async (token, exchangeRate, name='') => {
      await oracle.setRate(token.address, exchangeRate);
    ...
    await oracle.setPayroll(payroll.address);
    await oracle.setRate(etherToken.address, etherExchangeRate);
    ...
    await oracle.setRate(usdToken.address, 999);
    ...
    await setAndCheckRate(erc20Token1, erc20Token1ExchangeRate, "ERC20 Token 1");
    await setAndCheckRate(erc20Token2, erc20Token2ExchangeRate, "ERC 20 Token 2");
    await setAndCheckRate(erc677Token1, erc677Token1ExchangeRate, "ERC 677 Token 1");
  ...
  function logPayroll(salary, initialBalancePayroll, initialBalanceEmployee, payed, newBalancePayroll, newBalanceEmployee, expectedPayroll, expectedEmployee, name='') {
    console.log("");
  ...
  it("Test payday no token allocation", async () => {
    ...
    //let etherTokenFunds = web3.toWei(90, 'ether');
    ...
    await etherToken.wrap({from: funded_account, value: etherTokenFunds});
    await etherToken.transfer(owner, etherTokenFunds, {from: funded_account});
  ...
  it("Test payday zero exchange rate token", async () => {
    ...
    oracleFail.setPayroll(payroll2.address);
    await oracleFail.setRate(etherToken.address, 0);
    // Zero exchange rate
    return assertRevert(async () => {
      await payroll2.payday({from: employee1_1});
    });
  ...
  it("Test payday, time condition", async () => {
    ...
    await oracle.setRate(etherToken.address, etherExchangeRate);
    await oracle.setRate(usdToken.address, 100);
    await oracle.setRate(erc20Token1.address, erc20Token1ExchangeRate);
    await oracle.setRate(erc677Token1.address, erc677Token1ExchangeRate);
    // correct payday
    await payroll2.payday({from: employee1_1});
  ...
  it("Test EscapeHatch", async () => {
    ...
    const addInitialBalance = async (token, name="") => {
    ...
    await addInitialBalance(etherToken, "Ether Token");
    await addInitialBalance(usdToken, "USD Token");
    await addInitialBalance(erc20Token1, "ERC20 Token 1");
    await addInitialBalance(erc20Token2, "ERC20 Token 2");
    await addInitialBalance(erc677Token1, "ERC677 Token 1");
    await addInitialBalance(erc677Token2, "ERC677 Token 2");
    // Escape Hatch
    let transaction = await payroll2.escapeHatch();
    ...
    assert.equal(employerInitialBalance.sub(txFee).add(payrollTokenBalances[etherToken.address]).toString(), employerDestructedBalance.toString(), "Funds not recovered (Employer)!");
  ...
  it("Test payday, WITH Token allocation", async () => {
    ...
    // check that time restrictions for payday and determinAllocation work in a positive way (i.e. when time has gone by)

    // payday
    // set time forward, 1 month
    let newTime = parseInt(await payroll.getTimestampPublic()) + 2678400; // 31 * 24 * 3600
    console.log("Payroll time: " + await payroll.getTimestampPublic());
    await payroll.mock_setTimestamp(newTime); // 31 * 24 * 3600
    // TODO
    await finance.mock_setTimestamp(newTime);
    console.log("Payroll time: " + await payroll.getTimestampPublic());
    await setInitialBalances();
    // call payday again
    let transaction2 = await payroll.payday({from: employee2});
    // TODO: need to change finance time first
